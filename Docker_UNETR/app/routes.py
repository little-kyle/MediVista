...
from stroke.Multi_Modal_MRI_Segmentation import generate_rtstruct_segmentation_stroke
...
#从前端根据输入的患者信息对患者进行信息建档；
@app.route('/segmentation_stroke', methods=['POST'])
def segmentation_stroke():
    #data = request.get_json()
    #input
    dicom_files = []
    for key in request.files.keys():
    ...
    symbol=1  #默认使用单模态进行预测；
    rtstruct, isFromCurrentRTStruct = generate_rtstruct_segmentation_stroke(dicom_files, symbol, rtstruct_data)
    #print(type(rtstruct))
    buffer = rtstruct.save_to_memory()  # 字节流
