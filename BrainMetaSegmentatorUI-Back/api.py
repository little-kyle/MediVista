...
from segmentation import generate_rtstruct_segmentation_unetr, extract_roi_info,dicom_to_nifti_in_memory,dicom_to_binary
...
#注意端口修改
UNETR_URL='http://localhost:5000' 
#model_path = "C:\MetIA\BrainMetaSegmentatorUI-Back\model\checkpoint-epoch=1599-val_loss=0.225.ckpt"
load_dotenv()
model_path = "E:/VisualSystem/Front_Back/BrainMetaSegmentatorUI-Back/unetr/pretrained_models/UNETR_model_best_acc.pth"#os.getenv('MODEL_PATH')
...
def inputdata_to_binary(dicom_data, rtstruct_data):
    dicom_binary = [dicom_to_binary(dicom) for dicom in dicom_data]#dicom_to_binary(dicom_data)
    if rtstruct_data is not None:
        rtstruct_binary=dicom_to_binary(rtstruct_data)
    else:
        rtstruct_binary=None
    ...
    files={}
    for idx, sindicom_binary in enumerate(dicom_binary):
        files[f'dicom_data_{idx}'] = (f'dicom_data_{idx}.dcm', sindicom_binary, 'application/octet-stream')

    if rtstruct_binary is not None:
        files['rtstruct_data'] = ('rtstruct_data.dcm', rtstruct_binary, 'application/octet-stream')
    return files
